...
const string SIGFOX_DEVICE = "/dev/ttyUSB0"; // Path to the Sigfox device
const int BAUD_RATE = 9600; // Baud rate of the Sigfox module
...
void sendSigfoxData(const string& data) {
    try {
        // Open the serial port
        ofstream sigfoxDevice(SIGFOX_DEVICE);
        if (!sigfoxDevice.is_open()) {
            throw runtime_error("Failed to open " + SIGFOX_DEVICE);
        }

        // Set baud rate
        sigfoxDevice << "BAUD=" << BAUD_RATE << endl;

        // Send data
        sigfoxDevice << data << endl;
...
int main() {
   ...
   /* for (int i = 0; ports[i]; i++) {
    ...
    try {
        readFrame(port, totalAmount, prixBaguette, baguetteStock);
